...
// ─── Native context
// ─────────────────────────────────────────────────────────── GTA V passes args
// and receives results through a context struct Max 32 args + 4 result slots
// (from RAGE internals)
...
public:
  ...
  /// PATH B diagnostic: hex-dump the suspected native table so we can identify
  /// its real structure from live data instead of guessing.
  void DumpNativeTableRaw();
...
// ───────────────────────────────────────────────────── Source:
// https://github.com/alloc8or/gta5-nativedb-data Full list: include alloc8or's
// natives.h when building
namespace Natives {
// PLAYER
